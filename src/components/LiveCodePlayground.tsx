...
export const LiveCodePlayground: React.FC<Props> = ({ mode, token, onRunState }) => {
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (data.event === 'connection-failed' || data.event === 'disconnected' || data.event === 'runtime-error') { setNotice('Preview/SDK gặp lỗi'); onRunState('event-error', 'SDK đã ngắt kết nối hoặc không thể kết nối.'); }
    };
