...
export const LiveCodePlayground: React.FC<Props> = ({ mode, token, onRunState }) => {
  ...
  const run = () => {
    ...
    const rendered = code
      ...
      .replaceAll('{{VBOT_API_BASE_URL}}', escapeAttribute(import.meta.env.VITE_API_BASE_URL || 'https://open-api-staging.vbot.vn/v3.0'))
  ...
  return <section className="grid xl:grid-cols-2 gap-5 min-h-[680px]">
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col min-h-[680px]">
      ...
      <textarea value={code} onChange={e => setCode(e.target.value)} spellCheck={false} className="flex-1 min-h-[530px] resize-y p-4 font-mono text-xs leading-5 bg-slate-950 text-slate-200 outline-none" aria-label="HTML code editor" />
      <div className="p-3 border-t border-slate-100 flex flex-wrap gap-2 justify-between items-center"><span className="text-xs text-slate-500">{notice}</span><div className="flex gap-2"><button onClick={() => { setCode(getLiveDemoTemplate(mode, sdkBundleUrl)); setNotice('Đã đặt lại mẫu.'); }} className="px-3 py-2 text-xs font-bold rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50">Đặt lại mẫu</button><button onClick={copy} className="px-3 py-2 text-xs font-bold rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50">Sao chép code</button><button onClick={run} disabled={!token} className="px-3 py-2 text-xs font-bold rounded-lg bg-sky-600 hover:bg-sky-700 disabled:bg-slate-300 text-white"><Icon icon="solar:play-bold" className="inline mr-1" />Chạy code</button></div></div>
    </div>
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden min-h-[680px] flex flex-col"><div className="p-4 border-b border-slate-100"><h2 className="font-bold text-slate-800 flex gap-2 items-center"><Icon icon="solar:monitor-bold" className="text-emerald-600" /> Preview UI</h2><p className="text-xs text-slate-500 mt-1">SDK cần same-origin và quyền microphone để dùng localStorage/WebRTC. Chỉ chạy code bạn tin cậy.</p></div>{srcDoc ? <iframe ref={frameRef} title="VBot SDK live preview" sandbox="allow-scripts allow-same-origin" allow="microphone" srcDoc={srcDoc} className="w-full flex-1 min-h-[600px] border-0 bg-slate-50" /> : <div className="flex-1 p-8 flex flex-col justify-center items-center text-center text-slate-400"><Icon icon="solar:code-square-bold" className="text-4xl mb-3 text-slate-300" /><p className="font-semibold">Preview sẽ xuất hiện ở đây</p><p className="text-xs mt-1">Hoàn tất cấp token, sau đó bấm “Chạy code”.</p></div>}</div>
  </section>;
};
