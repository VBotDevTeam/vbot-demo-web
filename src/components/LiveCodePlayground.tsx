...
export const LiveCodePlayground: React.FC<Props> = ({ mode, token, onRunState }) => {
  ...
  const [isSdkSnippetCopied, setIsSdkSnippetCopied] = useState(false);
  ...
  const copySdkSnippet = async () => {
    ...
    try {
      ...
      setNotice('Đã sao chép mã ghép SDK kèm token.');
      setIsSdkSnippetCopied(true);
      if (copyFeedbackTimerRef.current) window.clearTimeout(copyFeedbackTimerRef.current);
      copyFeedbackTimerRef.current = window.setTimeout(() => setIsSdkSnippetCopied(false), 2000);
    } catch { setNotice('Không thể sao chép trên trình duyệt này.'); }
  };
  ...
  return <section className="grid xl:grid-cols-[minmax(430px,.72fr)_minmax(0,1.28fr)] gap-5 items-stretch">
    ...
    <section className="order-1 bg-white border border-slate-200 rounded shadow-xs overflow-hidden flex flex-col min-h-[900px]">
      ...
      <div className="p-4 border-b border-slate-100 bg-sky-50/40"><div className="flex flex-wrap gap-2 items-start justify-between mb-2"><div><h3 className="text-xs font-extrabold text-slate-800 uppercase tracking-wide">1. Mã ghép SDK</h3><p className="text-[11px] text-slate-500 mt-1">Token hiển thị trực tiếp sau khi Step 3 thành công.</p></div><button onClick={copySdkSnippet} disabled={!token} className={`px-3 py-2 text-xs font-bold rounded-lg text-white transition-all duration-200 disabled:bg-slate-300 ${isSdkSnippetCopied ? 'bg-emerald-600 hover:bg-emerald-700 scale-105' : 'bg-sky-600 hover:bg-sky-700'}`}><Icon icon={isSdkSnippetCopied ? 'solar:check-circle-bold' : 'solar:copy-bold'} className={`inline mr-1 ${isSdkSnippetCopied ? 'animate-bounce' : ''}`} />{isSdkSnippetCopied ? 'Đã sao chép!' : 'Sao chép mã SDK'}</button></div><textarea readOnly value={sdkSnippet} spellCheck={false} className="w-full h-52 resize-y p-3 rounded-lg border border-sky-100 font-mono text-xs leading-5 bg-slate-950 text-slate-100 outline-none" aria-label="SDK integration code" /></div>
